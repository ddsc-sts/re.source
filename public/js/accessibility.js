(() => {
  ...
  const dictionary = {
    ...
    es: { accessibility:'Accesibilidad', accessibility_options:'Opciones de accesibilidad', site_language:'Idioma del sitio', text_size:'Tamaño del texto', color_vision:'Percepción del color', high_contrast:'Alto contraste', readable_font:'Fuente de alta legibilidad', reduce_motion:'Reducir animaciones', open_guide:'Abrir guía del sitio', reset_preferences:'Restaurar preferencias', guide_title:'Cómo usar Re.Source', guide_intro:'Elige lo que quieres hacer. Puedes volver a esta guía desde el botón de accesibilidad.', find_materials:'Encontrar materiales', find_materials_desc:'Busca por categoría, ubicación y disponibilidad.', publish_material:'Publicar material', publish_material_desc:'Registra un residuo o insumo para reutilización.', negotiate:'Negociar con seguridad', negotiate_desc:'Sigue propuestas y mensajes en un solo lugar.', impact:'Ver tu impacto', impact_desc:'Consulta indicadores ambientales e informes ESG.', help_center:'Centro de ayuda', help_center_desc:'Encuentra respuestas y canales de soporte.', close_guide:'Cerrar guía',
  ...
  function apply(save = true) {
    ...
    if (save) localStorage.setItem(storeKey, JSON.stringify(prefs));
  }
  ...
  function init() {
    ...
    apply(false);
    window.addEventListener('load',()=>{ if(window.VLibras && !window.__resourceVLibras){ window.__resourceVLibras=true; new window.VLibras.Widget('https://vlibras.gov.br/app'); } });
  }
