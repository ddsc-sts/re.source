...
<body>
  ...
  <main>
    <section class="card">
      ...
      <h1>Cadastro recebido, <?= htmlspecialchars($companyName, ENT_QUOTES, 'UTF-8') ?>.</h1>
      <p class="lead">Sua empresa já pode navegar pelo catálogo e completar os dados da conta. Publicação, chat, negociação, frete e saque serão liberados assim que um administrador aprovar o cadastro.</p>
      <?php if ($notice !== ''): ?>
        <div class="notice"><?= htmlspecialchars($notice, ENT_QUOTES, 'UTF-8') ?></div>
      <?php endif; ?>
      <div class="actions">
        <a class="action" href="/re.source/busca"><strong>Explorar anúncios</strong><span>Conheça os materiais disponíveis.</span></a>
        <a class="action" href="/re.source/conta"><strong>Completar perfil</strong><span>Revise os dados da sua empresa.</span></a>
        <a class="action" href="/re.source/configuracoes"><strong>Configurações</strong><span>Ajuste tema e notificações.</span></a>
      </div>
      <a class="refresh" href="/re.source/aguardando-aprovacao">Verificar aprovação</a>
    </section>
  </main>
</body>
</html>
